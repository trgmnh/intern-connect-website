...
const Heading = ({ h2sub, h1main, paragraph, bgColor }) => {
    return (
        <div className={`flex flex-col items-center gap-1 py-8 px-5 text-center cursor-default ${bgColor}`}>
            ...
            <h1 className="text-2xl font-semibold md:font-semibold text-[#21272A] tracking-normal">
                {h1main}
            </h1>
            <div className="hidden md:flex items-center w-full max-w-xs gap-2 mt-1">
            ...
            {paragraph && <p className="mt-4 max-w-3xl text-sm text-gray-600">{paragraph}</p>}
        </div>
    );
}
...
export const HeadingNews = () => {
    const { language } = useLang();
    const t = ServicesHeading.news[language] || {};
    return <Heading h2sub={t.sub} h1main={t.main} />;
}
